...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!pdfText || !prompt) {
      return NextResponse.json({ error: "pdfText and prompt are required" }, { status: 400 });
    }
...
Voce receberá o texto extraído de um PDF de programacao de rodeio e um pedido do usuario.
    ...
    try {
      ...
      if (jsonMatch) {
        try {
          ...
          return NextResponse.json({ error: "Failed to parse AI response as JSON", rawResponse: rawContent }, { status: 500 });
        }
      } else {
        return NextResponse.json({ error: "AI did not return valid JSON", rawResponse: rawContent }, { status: 500 });
      }
    }
