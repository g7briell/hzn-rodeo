...
// 2. Fix the onClick in Ranking
const rankingClickSearch = `setSelectedPeaoProfile(data[0]);
...
const rankingClickReplace = `setSelectedPeaoProfile(data[0]);
                                setIsProfileModalOpen(true);`;

content = content.split(rankingClickSearch).join(rankingClickReplace);

// 3. Fix the onClick in Competidores Tab
const compClickSearch = `setSelectedPeaoProfile(data[0]);
                          setPublicEventSlug(null);
                          setPublicBoiadaSlug(null);
                          setPublicProfileSlug(peao.nome.trim().toLowerCase().replace(/\\s+/g, ''));
                          window.history.pushState({}, '', '/perfil/' + peao.nome.trim().toLowerCase().replace(/\\s+/g, ''));`;

const compClickReplace = `setSelectedPeaoProfile(data[0]);
                          setIsProfileModalOpen(true);`;

content = content.split(compClickSearch).join(compClickReplace);
...
content = content.replace('{/* ==================================== */}\n        {/* MODAL DE CADASTRO */}', modalJsx + '\n        {/* ==================================== */}\n        {/* MODAL DE CADASTRO */}');

fs.writeFileSync(file, content, 'utf8');
console.log('App.tsx patched to use profile modal!');
